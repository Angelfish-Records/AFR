...
export function mapRecordingRecord(
  ...
  return {
    ...
    oneStopStatus: asString(fields["One-Stop Status"]),
    explicitFlag: asString(fields["Explicit Flag"]),
    familyRecordingTypes: asStringArray(
      fields["Family Recording Types [Lookup]"],
    ),
    stemsAvailable: asString(fields["Stems Available"]),
    sampleClearanceStatus: asString(fields["Sample Clearance Status"]),
    ...
    bpm: asNumber(fields.BPM),
    musicalKey: asString(fields.Key),
    timeSignature: asString(fields["Time Signature"]),
