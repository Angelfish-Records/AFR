...
export default function CampaignComposerPage() {
  ...
  const [approach, setApproach] = useState<string>(""); // '' means All
  ...
  const [approachOptions, setApproachOptions] = useState<string[]>([]);
  const [priorityOptions, setPriorityOptions] = useState<string[]>([]);

  const [senderKey, setSenderKey] = useState<SenderKey>("angus");

  const sender = useMemo(
    () => SENDERS[senderKey] ?? SENDERS.brendan,
    [senderKey],
  );
  const replyTo = sender.replyTo;

  const [subjectTemplate, setSubjectTemplate] = useState(
    "News from Angelfish Records",
  );
...
`,
  ...
  const picked = useMemo(() => {
    const c =
      sampleContacts.find((x) => x.id === samplePick) ?? sampleContacts[0];
    return c ?? null;
  ...
  const previewSubject = useMemo(
    () => mergeTemplate(subjectTemplate, previewVars),
    [subjectTemplate, previewVars],
  );
  const previewBody = useMemo(
    () => mergeTemplate(bodyTemplate, previewVars),
    [bodyTemplate, previewVars],
  );
  ...
  const labelTitleStyleLeft: React.CSSProperties = {
    fontSize: 10,
    opacity: 0.7,
    marginBottom: 6,
  };
  const labelTitleStyleRight: React.CSSProperties = {
    fontSize: 12,
    opacity: 0.7,
    marginBottom: 6,
  };
    ...
    try {
      ...
      const res = await fetch("/api/campaigns/preview", {
        ...
        body: JSON.stringify({
          ...
          logoUrl:
            "https://www.angelfishrecords.com/brand/AFR_logo_circle_light_mini.png",
          recipientName,
  ...
  async function loadAudience() {
    ...
    try {
      ...
      setAudienceCount(
        typeof data.mailableCount === "number" ? data.mailableCount : null,
      );
      setSampleContacts(
        Array.isArray(data.sampleContacts) ? data.sampleContacts : [],
      );

      setOutletTypeOptions(
        Array.isArray(data.availableOutletTypes)
          ? data.availableOutletTypes
          : [],
      );
      setOutletRegionOptions(
        Array.isArray(data.availableOutletRegions)
          ? data.availableOutletRegions
          : [],
      );
      setApproachOptions(
        Array.isArray(data.availableApproaches) ? data.availableApproaches : [],
      );
      setPriorityOptions(
        Array.isArray(data.availablePriorities) ? data.availablePriorities : [],
      );

      if (data.sampleContacts?.[0]?.id)
        setSamplePick(data.sampleContacts[0].id);
    } catch (e: unknown) {
  ...
  useEffect(() => {
    loadAudience();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [outletType, outletRegion, approach, priority]);
  ...
  async function sendAutoDrain(opts?: { limit?: number; maxLoops?: number }) {
    ...
    const maxLoops = Math.max(
      1,
      Math.min(50, Math.floor(opts?.maxLoops ?? 50)),
    );
    ...
    try {
      ...
      while (loops < maxLoops && remainingQueued > 0) {
        ...
        remainingQueued =
          typeof data.remainingQueued === "number"
            ? data.remainingQueued
            : remainingQueued;
        ...
        const nextPollMs =
          ...
          Number.isFinite(data.nextPollMs)
      ...
      <div style={{ padding: 12 }}>
        ...
        >
          ...
          {sendStatus.state === "cancelled" && (
            <div style={{ fontSize: 12 }}>
              <b>Cancelled.</b> Sent <b>{sendStatus.totalSent}</b> before
              stopping.
            </div>
          )}
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Internal tool for composing and sending AFR press campaigns."
        />
      </Head>
      <div
        style={{
          ...
          <div style={{ padding: 12, borderRadius: 12 }}>
            ...
            <div>
              <div style={labelTitleStyleRight}>
                Rendered plaintext (what you store in Campaign body)
              </div>
              <pre
                style={{
                  whiteSpace: "pre-wrap",
