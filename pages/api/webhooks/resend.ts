...
async function airtableUpsertByUniqueField(args: {
  ...
  const filter = encodeURIComponent(
    `{${uniqueField}} = "${escapeAirtableString(uniqueValue)}"`,
  );
  const searchUrl = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(table)}?maxRecords=1&filterByFormula=${filter}`;

  const found = await airtableRequestWithRetry<{
    records?: Array<{ id: string }>;
  }>(token, searchUrl);
  if (!found.ok)
    throw new Error(`Airtable search failed: ${found.status} ${found.body}`);
  ...
  if (!write.ok)
    throw new Error(`Airtable write failed: ${write.status} ${write.body}`);
  return { created: !existingId };
}
...
async function airtableFindPressContactIdByEmail(args: {
  ...
  const filter = encodeURIComponent(
    `LOWER({Email}) = "${escapeAirtableString(email)}"`,
  );
  const url = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(contactsTable)}?maxRecords=1&filterByFormula=${filter}`;

  const res = await airtableRequestWithRetry<{
    records?: Array<{ id: string }>;
  }>(token, url);
  if (!res.ok)
    throw new Error(
      `Airtable contact lookup failed: ${res.status} ${res.body}`,
    );
...
async function airtableSuppressionExists(args: {
  ...
  if (!res.ok)
    throw new Error(
      `Airtable suppression lookup failed: ${res.status} ${res.body}`,
    );
...
async function airtableCreateSuppression(args: {
  ...
  if (!res.ok)
    throw new Error(
      `Airtable suppression create failed: ${res.status} ${res.body}`,
    );
}

function mapDeliveryStatus(
  type: string,
): "Sent" | "Delivered" | "Bounced" | "Complained" | "Failed" | null {
  switch (type) {
    case "email.sent":
      return "Sent";
...
async function airtableUpdateSendByResendMessageId(args: {
  ...
  if (!found.ok)
    throw new Error(
      `Airtable send lookup failed: ${found.status} ${found.body}`,
    );
  ...
  if (!res.ok)
    throw new Error(`Airtable send update failed: ${res.status} ${res.body}`);
}

async function airtableIncrementClickedCountByResendMessageId(args: {
  token: string;
  baseId: string;
  sendsTable: string;
  resendMessageId: string;
  ...
  const { token, baseId, sendsTable, resendMessageId, lastEventAtIso } = args;
  ...
  const found = await airtableRequestWithRetry<{
    records?: Array<{ id: string; fields?: { "Clicked count"?: number } }>;
  }>(token, url);
  ...
  const send = found.json.records?.[0];
  if (!send) return;

  const currentCount =
    typeof send.fields?.["Clicked count"] === "number" &&
    Number.isFinite(send.fields["Clicked count"])
      ? send.fields["Clicked count"]
      : 0;

  const patchUrl = `https://api.airtable.com/v0/${baseId}/${encodeURIComponent(sendsTable)}/${send.id}`;
  const res = await airtableRequestWithRetry<unknown>(token, patchUrl, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      fields: {
        "Clicked count": currentCount + 1,
        "Last event at": lastEventAtIso,
      },
    }),
  });
  ...
  if (!res.ok)
    throw new Error(
      `Airtable click count update failed: ${res.status} ${res.body}`,
    );
}
...
export default async function handler(
  ...
  const resendEventsTable = must(
    process.env.AIRTABLE_RESEND_EVENTS_TABLE,
    "AIRTABLE_RESEND_EVENTS_TABLE",
  );
  const pressContactsTable = must(
    process.env.AIRTABLE_PRESS_CONTACTS_TABLE,
    "AIRTABLE_PRESS_CONTACTS_TABLE",
  );
  const suppressionsTable = must(
    process.env.AIRTABLE_SUPPRESSIONS_TABLE,
    "AIRTABLE_SUPPRESSIONS_TABLE",
  );
  ...
  try {
    ...
    const eventCreatedAtIso = event.created_at
    ...
    console.log("[resend-webhook] verified", {
      svixId,
      type,
      to: toEmail ? `sha:${emailHash(toEmail)}` : "",
    });

    // 1) Immutable event log (idempotent by svix_id)
    const eventLogWrite = await airtableUpsertByUniqueField({
      baseId,
    ...
    if (type === "email.clicked" && resendMessageId) {
      await airtableIncrementClickedCountByResendMessageId({
        token: airtableToken,
        baseId,
        sendsTable,
        resendMessageId,
